...
class multiheadNet(nn.Module):
    def __init__(self, featurizer, classifier1, classifier2):
    ...
    def set_noise(self, noiseset='initial'):
        ...
        self.featurizer.set_noise(noiseset)

    def set_grad_eps(self, grad_eps=1.0):
        self.featurizer.set_gradeps(grad_eps)

    def set_drop_eps(self, drop_eps=0.0):
        self.droprate = drop_eps
...
class DFP(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DFP, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        self.num_train = num_domains
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        ...
        self.network = multiheadNet(self.featurizer, self.classifier, self.classifier2)
        self.network = torch.nn.DataParallel(self.network).cuda()
        ...
        """
                params = [{"params":self.featurizer.parameters(),"lr":self.hparams["lr"],"weight_decay":self.hparams['weight_decay']},
                          {"params": self.classifier.parameters(), "lr": self.hparams["lr"], "weight_decay": self.hparams['weight_decay']},
                          {"params": self.classifier2.parameters(), "lr": 1e-6, "weight_decay": 5e-3},
                          ]

                self.optimizer = _get_optimizer(
                    self.hparams.get('optimizer', 'adam'), #'adam'  'sgdm'
                    params,
                    lr=self.hparams["lr"],
                    weight_decay=self.hparams['weight_decay']
                )
    ...
    def update(self, minibatches, grad_eps, loss1_eps, loss2_eps, drop_eps, unlabeled=None):
        all_x = torch.cat([x for x, y in minibatches])
        all_y = torch.cat([y for x, y in minibatches])

        minibatch = all_x.shape[0] // self.num_train

        domain_label = []
        for i in range(0, self.num_train):
            dlabeli = torch.cuda.LongTensor(minibatch)
            torch.ones(minibatch, out=dlabeli)
            domain_label.append(i*dlabeli)

        dlabel = torch.cat([x for x in domain_label])

        all_x.requires_grad = True

        self.network.module.set_grad_eps(grad_eps)
        self.network.module.set_drop_eps(drop_eps)
        self.network.module.set_noise("initial")
        predict1, predict2 = self.predict(all_x)
        loss2 = F.cross_entropy(predict2, dlabel)

        self.optimizer.zero_grad()
        loss2.backward()

        self.network.module.set_noise("addnoise")
        npredict1, npredict2 = self.predict(all_x)
        nloss1 = F.cross_entropy(npredict1, all_y)
        nloss2 = F.cross_entropy(npredict2, dlabel)
        loss = loss1_eps * nloss1 + loss2_eps * nloss2
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
